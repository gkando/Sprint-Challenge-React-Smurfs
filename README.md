...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
        ...
        Server-side Routing: 
                              - Any link the user clicks on sends a request to the server for the page.
                              - The server must then locate and return the sought after information.
                              - The browser must reload to render the newly received information.
                              - Benefit is the user only receives the information they are seeking. 
                              - Downside is page refresh and load time of new page.
        Client-side Routing:
                              - Route is handled internally by the JS that is loaded on the page.
                              - User clicks on link and content dynamically renders on page without having to request resources from the server.
                              - Generally faster because no data needs to be passed when routed from one page to another. 
                              - Downside is the whole website or web app needs to be loaded on the first request.
          
- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

      GET -- 'hey server can you send over {$foo} resource'

      PUT -- ' hey server can you change {$bar} for {$foo} resource'

      POST -- 'hey server can you please add this resource'

      DELETE -- 'hey server get the {$foo} outta here!'
